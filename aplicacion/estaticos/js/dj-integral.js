...
(function () {
  ...
  function alternarEmpresa(boton) {
    try {
      ...
      if (empresasExpandidasDJ.has(rut)) {
        ...
        empresasExpandidasDJ.delete(rut);
    ...
    try {
      ...
      filasEmpresas.forEach(fila => {
        ...
        if (mostrarFila) {
          ...
          if (empresasExpandidasDJ.has(rut)) empresasExpandidasDJ.delete(rut);
        }
      });
  ...
  // Exponer globalmente para uso en onclick desde otras páginas
  window.abrirModalObservacionesDJ = abrirModalObservacionesDJ;
