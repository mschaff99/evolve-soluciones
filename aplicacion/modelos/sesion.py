...
class SesionUsuario:
    ...
    @staticmethod
    def obtener_por_token(token_sesion: str) -> Optional['SesionUsuario']:
        """
        ...
        try:
            ...
            if resultado:
                # type: ignore - resultado es un dict válido de PostgreSQL
                return SesionUsuario(
                    id=resultado['id'],  # type: ignore
                    token_sesion=resultado['token_sesion'],  # type: ignore
                    id_usuario=resultado['id_usuario'],  # type: ignore
                    direccion_ip=resultado['direccion_ip'],  # type: ignore
                    user_agent=resultado['user_agent'],  # type: ignore
                    fecha_creacion=resultado['fecha_creacion'],  # type: ignore
                    fecha_ultima_actividad=resultado['fecha_ultima_actividad'],  # type: ignore
                    activa=resultado['activa']  # type: ignore
                )
            return None
        except Exception as e:
    ...
    @staticmethod
    def crear_sesion_unica(id_usuario, direccion_ip, user_agent):
        """
        Crea una nueva sesión para un usuario cerrando todas las sesiones previas

        Implementa la lógica de "una sola sesión activa por usuario":
        ...
        Returns:
            SesionUsuario: Nueva instancia de sesión (única activa)
        """
        try:
            # Paso 1: Cerrar todas las sesiones activas existentes
            SesionUsuario.cerrar_todas_sesiones_usuario(id_usuario)
    ...
    @staticmethod
    def validar_sesion_unica(token_sesion):
        """
        Valida que la sesión sea la única activa para el usuario

        Args:
            token_sesion (str): Token de la sesión a validar

        Returns:
            bool: True si la sesión es válida y única, False si fue invalidada
        """
        try:
            ...
            # Verificar que no hay otras sesiones activas para el mismo usuario
            consulta = """
                SELECT COUNT(*) as count
                FROM auth.sesiones_usuario
                WHERE id_usuario = %s AND activa = TRUE AND token_sesion != %s
            """
            resultado = ejecutar_consulta_postgres(
                consulta,
                (sesion.id_usuario, token_sesion),
                obtener_uno=True
            )

            # type: ignore - resultado es un dict válido de PostgreSQL
            otras_sesiones_activas = resultado['count'] if resultado else 0  # type: ignore
            ...
            print(f"Error validando sesión única {token_sesion}: {e}")
            return False
