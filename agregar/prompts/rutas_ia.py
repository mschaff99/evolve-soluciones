...
from decorators.auth import admin_required, supervisor_required
import traceback
...
def probar_conexion_gemini():
    ...
    try:
        ...
        print(f"🧪 Usuario {current_user.username} probando conexión con Gemini")
...
def dashboard_ia():
    ...
    """
    print(f"📊 Usuario {current_user.username} (Rol: {current_user.rol}) accediendo al Dashboard IA")
...
def obtener_empresas_para_ia():
    ...
    """
    try:
        from services.empresa_service import obtener_todas_las_empresas_completo

        anio = request.args.get('anio', type=int)
        mes = request.args.get('mes', type=int)

        print(f"🏢 Obteniendo empresas para IA: anio={anio}, mes={mes}")
        print(f"👤 Usuario: {current_user.username}, Rol: {current_user.rol}")

        # TODOS los usuarios pueden ver TODAS las empresas (sin restricciones)
        data = obtener_todas_las_empresas_completo()
        ...
        if data.get('success'):
            ...
            for empresa in empresas:
                empresa_formateada = {
                    ...
                    'usuario': empresa.get('usuario', 'Sin asignar'),
            ...
            return jsonify({
                'success': False,
                'error': 'No se pudieron obtener las empresas',
                'empresas': []
            }), 500
...
def obtener_historial_analisis(empresa_rut):
    ...
    """
    try:
        from services.ia_memoria_service import IAMemoriaService
